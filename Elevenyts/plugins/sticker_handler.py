...
def _render_universal_sticker(content_text: str, is_sticker: bool, user_name: str, pfp_path: str | None, out_path: str):
    ...
    if is_sticker:
        ...
        text_color = (130, 160, 180, 255) # Light gray-blue
        
    # Text wrapping logic (Handles wide strings beautifully)
    lines = []
    ...
    if pfp_path and os.path.exists(pfp_path):
        ...
        # Default dynamic colored avatar placeholder
    ...
    # 2. Rounded Message Bubble Drawing
    ...
    # 3. Render Name and Message content
    draw.text((bx1 + padding, by1 + padding), user_name, fill=name_color, font=font_name)
    
    # Yaha text draw ho raha hai, agar normal font fail ho toh fallback font automatically run hoga
    try:
    ...
    # Scale canvas to standard telegram format 512x512
...
async def process_universal_sticker(message: Message) -> str | None:
    ...
    if user and user.photo:
        try:
            pfp_path = await message.app.download_media(user.photo.big_file_id)
        except Exception:
            pfp_path = None
